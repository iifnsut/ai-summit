...
export default function Home() {
  ...
  const headerBorder = useTransform(
    homeScroll.scrollYProgress,
    [0, 1],
    ["hsla(var(--border) / 1)", "hsla(var(--border) / 0)"]
  );
  const headerBgBlur = useTransform(
    homeScroll.scrollYProgress,
    [0, 1],
    ["blur(var(4px))", "blur(0px)"]
  );
  ...
  const updateTabs = (e: IntersectionObserverEntry | null) => {
    setTabs(
      tabs.map((tab) => ({ ...tab, active: `#${e?.target.id}` === tab.href }))
    );
  };
  return (
    <>
      ...
      <Box
        ...
        id="home"
        className="h-screen w-full flex justify-center items-center text-foreground"
      >
        <motion.div
          ref={homeRef}
          initial={{ opacity: 0 }}
      ...
      >
        ...
        >
          ...
          <div className="bg-background relative p-6 md:p-16 rounded-3xl pointer-events-none text-justify">
            Step into the Delhi AI4Humanity Summit 2025, where NSUT IIF and the
            Embassy of Israel bring together innovators, thinkers, and
            changemakers to harness AI for a better world. Over a month of
            hackathons, competitions, and inspiring talks, explore how AI is
            revolutionizing healthcare, agriculture, and communities, creating a
            future that empowers all.
          </div>
        </motion.div>
      </Box>
      <Box
      ...
      <Box
        onViewportEnter={updateTabs}
        id="timeline"
        className="w-full h-screen flex flex-col items-center justify-center bg-background"
      >
      ...
      <Box
        onViewportEnter={updateTabs}
        id="FAQs"
        className="w-full gap-8 h-screen flex flex-col items-center justify-center bg-background"
      >
